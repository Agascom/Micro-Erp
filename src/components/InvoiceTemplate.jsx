...
const InvoiceTemplate = ({ invoice }) => {
    ...
    const totalTVA = 0; // Assuming 0 for now
    const shipping = 0;
    const totalTTC = subTotal + totalTVA + shipping; // Or use invoice.total

    return (
        <div className="bg-gray-50 font-sans p-4 sm:p-6 md:p-8 rounded-xl" id="invoice-preview">
            <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-lg p-6 sm:p-8 md:p-12">
                {/* Header */}
                <header className="flex flex-col sm:flex-row justify-between items-start pb-8 border-b border-gray-200">
                    ...
                    <div className="mt-6 sm:mt-0 text-left sm:text-right">
                        <h1 className="text-3xl md:text-4xl font-bold text-gray-800 tracking-wider uppercase">FACTURE N°:</h1>
                        <p className="text-2xl md:text-3xl font-bold text-gray-600 tracking-wider mt-2">{invoice.invoice_number}</p>
                    </div>
                </header>
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 gap-8 my-8">
                    <div className="bg-cyan-50 p-6 rounded-lg">
                        ...
                        <DetailRow label="Date d'échéance :" value={new Date(invoice.due_date).toLocaleDateString('fr-FR')} />
                ...
                {/* Footer */}
                <footer className="pt-8 mt-8 border-t border-gray-200 text-sm text-gray-600">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="bg-gray-50 p-6 rounded-lg md:col-span-1">
                            <h4 className="font-bold text-gray-800 mb-2">Banque:</h4>
                            <p>Numéro de compte: FR76 ....</p>
                            <p>IBAN: FR76 ....</p>
                            <p>SWIFT BIC: ....</p>
                        </div>
                        <div className="bg-gray-50 p-6 rounded-lg md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-8">
                            <div>
